...
- **L1.3 — shard the EVAL_DESIGN phase** — **NEXT (designed — resume here).** Replace the single monster `delegate` call in `_orchestrator_phases/eval_design.py` with **parallel per-resource architect calls + a Python merge**. Concrete design:
  ...
  - Tests: merge + prompt-building (unit); the LLM calls verify via the `derisk_eval_design` probe. **Per-resource is the default granularity.**
