...
| 6 | ✅ **Auto-skip resource after 2 consecutive cost rejections** — SHIPPED `dfa81d4` | MED | LOW | `cost_reject_streak` + `cost_unwinnable` status (mirrors F21 `unwinnable`); `CGF_MAX_COST_REJECTIONS=2`. After N consecutive `reject_cost` the resource is dropped from ITERATE (incumbent stands); rerun with relaxed τ. Unit-covered (`TestCostUnwinnable`). |
| 7 | ✅ **Raise `CGF_ITERATE_CONCURRENCY` 4 → 6** — SHIPPED `dfa81d4` | LOW | LOW (env knob) | Same model + rate window as GENERATE at 8-way. ~15–20 % wall-time win on ITERATE r1. |
...
**Carry-forward from § 3.6:** #6 (auto-skip after 2 cost rejections) + #7 (`CGF_ITERATE_CONCURRENCY` 4→6) **SHIPPED `dfa81d4`** (`cost_unwinnable` status + streak; unit-covered, rides the next organic smoke). Remaining: #5 GENERATE word-count guidance; defer #8 pre-flight dry-run, #9 per-type τ profile, #10 trials default behind A1–A4 and Phase D data.
...
- **L1.3 — shard the EVAL_DESIGN phase** — **SHIPPED + probe-verified (2026-06-13).** Replaced the single monster `delegate` call in `_orchestrator_phases/eval_design.py` with **parallel per-resource architect calls + a Python merge**. As built:
...
- **Measured (probe ✓ 2026-06-13, iac-team 18 resources, `derisk_eval_design.py` in-container):**
  - **Wall-time: 17m54s → 5m11s (3.45× faster).** 18/18 shards succeeded → 54 scenarios (18×3). At `CGF_EVAL_DESIGN_CONCURRENCY=5` the run is ~4 waves; slowest shards ~2m30s.
  - **Per-shard turns: 4 (one outlier at 8) vs the monolith's 74.** `max_turns=15` enforced with **0 cap-hits** — the inline diff means shards read little, so the cap is headroom, not a constraint.
  - **Discrimination held: 38 `llm_judge` graders (run #8 had 0), 18/18 resources carry a judge/quality grader, 0 trajectory-on-content** (A1 holds; the 9 trajectory assertions are all `constraint`, LLM-judged). Grader mix: contains 54, llm_judge 38, composite 29, trajectory 7.
  - **Open CHECK (orthogonal to L1.3):** the architect's `llm_judge` rubrics don't literally spell the 1–7 anchors (the judge still scores 1–7 via `llm_judge.py`'s system prompt, so scoring is unaffected). Follow-up: strengthen the rubric-anchoring instruction in `cgf-eval-architect.md`, or relax the probe's A3 literal-string check. A2 (discrimination audit) + A4 (cost floor) only fire during EXECUTION_EVAL — a full **smoke #9** is still needed for end-to-end promote-rate.
