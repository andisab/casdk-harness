...
_get_iteration_timeout = get_iteration_timeout
tg_get_iteration_timeout = get_iteration_timeout
TG_DEFAULT_TIMEOUT = DEFAULT_ITERATION_TIMEOUT
TG_TIMEOUT_ENV_VAR = ITERATION_TIMEOUT_ENV_VAR
...
class TestIterationTimeout:
    ...
    def test_default_iteration_timeout(self) -> None:
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            timeout = _get_iteration_timeout()
...
class TestTextGradIterationTimeout:
    """Tests for TextGrad iteration timeout configuration."""

    def test_default_timeout(self) -> None:
        """Test default TextGrad iteration timeout."""
        with patch.dict(os.environ, {}, clear=True):
            if TG_TIMEOUT_ENV_VAR in os.environ:
                del os.environ[TG_TIMEOUT_ENV_VAR]

            timeout = tg_get_iteration_timeout()
            assert timeout == TG_DEFAULT_TIMEOUT
            assert timeout == 1800

    def test_custom_timeout_from_env(self) -> None:
        """Test custom TextGrad timeout from environment."""
        with patch.dict(os.environ, {TG_TIMEOUT_ENV_VAR: "900"}):
            timeout = tg_get_iteration_timeout()
            assert timeout == 900.0

    def test_invalid_timeout_uses_default(self) -> None:
        """Test invalid TextGrad timeout uses default."""
        with patch.dict(os.environ, {TG_TIMEOUT_ENV_VAR: "not_a_number"}):
            timeout = tg_get_iteration_timeout()
            assert timeout == TG_DEFAULT_TIMEOUT
...
class TestProgressFormatting:
    ...
    def test_format_elapsed_boundary(self) -> None:
        ...
        # Just under a minute should still be seconds
        assert _format_elapsed(59.9) == "059.9s"
        # At exactly a minute should be minutes
        assert _format_elapsed(60.0) == "01m00s"


class TestTimeoutEnvVarConsistency:
    """Tests for env var consistency between optimizers."""

    def test_same_env_var_name(self) -> None:
        """Test that both optimizers use the same env var."""
        assert ITERATION_TIMEOUT_ENV_VAR == TG_TIMEOUT_ENV_VAR
        assert ITERATION_TIMEOUT_ENV_VAR == "CGF_ITERATION_TIMEOUT"

    def test_same_default_timeout(self) -> None:
        """Test that both optimizers have same default timeout."""
        assert DEFAULT_ITERATION_TIMEOUT == TG_DEFAULT_TIMEOUT
